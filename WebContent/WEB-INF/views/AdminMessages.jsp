...
<center>
	...
	<c:forEach items="${msgrecords}" var="record">
				<tr>
					<td>${record.name}</td>
					<td>${record.email}</td>
					...
					<c:if test="${!record.read}">
					<input name="markmsg" type="submit" value="Read" />
					</c:if>
					<c:if test="${record.read}">
					<input name="markmsg" type="submit" value="UnRead" />
					</c:if>
					<input name="id" type="hidden" value="${record.getId()}">
					</form>
	...
	<ul class="pagination">
		<c:if test="${currentPage != 1}">
			<li class="page-item"><a class="page-link"
				href="AdminMessage?recordsPerPage=${recordsPerPage}&currentPage=${currentPage-1}">Previous</a>
			</li>
		</c:if>
		...
		<c:forEach begin="1" end="${noOfPages}" var="i">
			<c:choose>
				...
				<c:otherwise>
					<li class="page-item"><a class="page-link"
						href="AdminMessage?recordsPerPage=${recordsPerPage}&currentPage=${i}">${i}</a>
					</li>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:if test="${currentPage lt noOfPages}">
			<li class="page-item"><a class="page-link"
				href="AdminMessage?recordsPerPage=${recordsPerPage}&currentPage=${currentPage+1}">Next</a>
			</li>
		</c:if>
	</ul>
	</nav>
